...
char point[1000][2000000];
char* Pic_read(char* file_name, char* buff) {
	FILE * fpPhoto;
	int     iRead;
	char    szBuf[100];
	struct stat st1;
	...
	long long total = 0;
	while ((iRead = fread(buff + total, 1, sizeof(szBuf), fpPhoto)) > 0) {
		total += iRead;
	}
...
int main(int args, char** argv) {
	...
	for (int i = 0; i < end_file - begin_file; i++) {
		...
		Pic_read(totalFile[i + begin_file].FilePath, point[i]);
		if (i % 100 == 0) {
			time(&end);
	...
	//CvMat mat = cvMat(2192, 1536, CV_8UC1, tempBuff);
